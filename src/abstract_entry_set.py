...
class AbstractEntrySet:
    ...
    def __str__(self) -> str:
        ...
        for entry in self:
            ...
            output.append(entry.to_string(indent=2, repr_format=False, extra_data=[f"parent={str(parent.id if parent else None)}"]))
        return "\n".join(output)
    ...
    def __repr__(self) -> str:
        ...
        for entry in self:
            parent: Optional[AbstractEntry]
            parent = self.get_parent(entry)
            output.append(entry.to_string(indent=2, repr_format=True, extra_data=[f"parent={repr(parent.id if parent else None)}"]))
        output.append("]")
        output.append(")")
        return "".join(output)
