...
class ComputedData:
    ...
    def __init__(
        ...
        for entry in self.__filtered_gain_loss_set:
            gain_loss = cast(GainLoss, entry)
            if not gain_loss.disposed_of_lot or gain_loss.disposed_of_lot.timestamp.year < from_year or gain_loss.disposed_of_lot.timestamp.year > to_year:
                continue
            self.__in_lot_sold_percentage[gain_loss.disposed_of_lot] = (
                self.__in_lot_sold_percentage.setdefault(gain_loss.disposed_of_lot, ZERO) + gain_loss.disposed_of_lot_fraction_percentage
            )
