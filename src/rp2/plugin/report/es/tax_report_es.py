...
# Just a copy of tax_report_en.py to initialize spain reports.
...
LOGGER: logging.Logger = create_logger("tax_report_es")
...
class Generator(AbstractODSGenerator):
    MIN_ROWS: int = 20
    MAX_COLUMNS: int = 20
    OUTPUT_FILE: str = "tax_report_es.ods"
    ...
    def generate(
        ...
        if from_date != MIN_DATE and to_date != MAX_DATE:
            raise RP2RuntimeError("To and From Dates can not be specified for the ES tax report.")
        ...
        template_path: str = self._get_template_path("tax_report_es", country, generation_language)
