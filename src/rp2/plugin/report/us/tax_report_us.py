...
class Generator(AbstractODTGenerator):
    ...
    def __generate(self, output_file: Any, asset: str, gain_loss_set: GainLossSet, row_indexes: Dict[str, int]) -> None:
        ...
        for entry in gain_loss_set:
            ...
            disposed_of_lot_note_vs: str = f"from_lot_note{border_suffix}"
            ...
            if gain_loss.disposed_of_lot:
                ...
                self._fill_cell(sheet, row_index, 2, gain_loss.disposed_of_lot.timestamp.strftime("%m/%d/%Y"), visual_style=disposed_of_lot_note_vs)
                self._fill_cell(sheet, row_index, 5, gain_loss.fiat_cost_basis, visual_style=disposed_of_lot_note_vs, data_style="fiat")
                self._fill_cell(sheet, row_index, 10, disposed_of_lot_note, visual_style=disposed_of_lot_note_vs)
            else:
