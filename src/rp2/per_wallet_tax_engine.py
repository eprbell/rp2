...
def _convert_per_wallet_transactions_to_input_data(
    configuration: Configuration, universal_input_data: InputData, per_wallet_transactions: _PerWalletTransactions
) -> InputData:
    ...
    result: InputData = InputData(
        ...
        universal_input_data.from_date,
        universal_input_data.to_date,
    )
    return result
...
def transfer_analysis(configuration: Configuration, transfer_semantics: AbstractAccountingMethod, universal_input_data: InputData) -> Dict[Account, InputData]:
    ...
    for transaction_set in [
        ...
        universal_input_data.unfiltered_out_transaction_set,
        universal_input_data.unfiltered_intra_transaction_set,
    ]:
    ...
    for wallet, per_wallet_transactions in wallet_2_per_wallet_transactions.items():
        per_wallet_input_data = _convert_per_wallet_transactions_to_input_data(configuration, universal_input_data, per_wallet_transactions)
