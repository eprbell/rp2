...
class TestGainLoss(unittest.TestCase):
    ...
    def test_good_interest_gain_loss(self) -> None:
        ...
        self.assertEqual(flow.disposed_of_lot, None)
  ...
  disposed_of_lot=None""",
    ...
    def test_good_non_interest_gain_loss(self) -> None:
        ...
        self.assertEqual(flow.disposed_of_lot, self._in_buy)
        self.assertEqual(flow.timestamp, flow.taxable_event.timestamp)
  ...
  disposed_of_lot_fiat_amount_with_fee_fraction=10.0100
  disposed_of_lot_fraction_percentage=0.0500%
  disposed_of_lot=InTransaction:
    ...
    def test_bad_gain_loss(self) -> None:
        ...
        with self.assertRaisesRegex(RP2TypeError, "Parameter 'disposed_of_lot' is not of type InTransaction: "):
            # Bad disposed_of lot
            GainLoss(self._configuration, self._accounting_method, RP2Decimal("0.1"), self._out, 33)  # type: ignore

        with self.assertRaisesRegex(
            RP2TypeError,
            "disposed_of_lot must be None for earn-typed taxable_events, instead it's foobar",
        ):
            # Bad disposed_of lot
        ...
        with self.assertRaisesRegex(
            ...
            # Earn-typed taxable event: disposed_of_lot not None
            GainLoss(self._configuration, self._accounting_method, RP2Decimal("1.1"), self._in_interest, None)

        with self.assertRaisesRegex(
            RP2TypeError,
            "disposed_of_lot must be None for earn-typed taxable_events, instead it's .*",
        ):
            # Earn-typed taxable event: disposed_of_lot not None
            GainLoss(self._configuration, self._accounting_method, RP2Decimal("0.1"), self._in_interest, self._in_buy2)

        with self.assertRaisesRegex(RP2TypeError, "disposed_of_lot must not be None for non-earn-typed taxable_events"):
            # Non-earn-typed taxable event: disposed_of lot None
            GainLoss(self._configuration, self._accounting_method, RP2Decimal("0.2"), self._out, None)

        with self.assertRaisesRegex(
            RP2ValueError,
            "crypto_amount .* is greater than taxable event amount .* or disposed-of lot amount .*: ",
        ):
            # Non-earn-typed taxable event: disposed_of_lot not None
            GainLoss(self._configuration, self._accounting_method, RP2Decimal("2"), self._out, self._in_buy2)

        with self.assertRaisesRegex(RP2ValueError, "Timestamp .* of taxable_event is earlier than timestamp .* of disposed_of_lot: .*"):
            # Non-earn-typed taxable event: disposed_of_lot not None
            GainLoss(self._configuration, self._accounting_method, RP2Decimal("0.1"), self._out, self._in_buy3)

        with self.assertRaisesRegex(RP2ValueError, "taxable_event.asset .* != disposed_of_lot.asset .*"):
